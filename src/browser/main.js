...
(function()
{
    ...
    function onload()
    {
        ...
        if (query_args["script"])
	{
            const user_script = document.createElement("script");
            user_script.src = "build/"+query_args["script"]+".js";
            user_script.async = true;
        ...
        {
	    console.log("Start Profile: "+infos)
            $("boot_options").style.display = "none";
    ...
    {
        console.log("Start Emulation: "+settings)
	...
	var cq = [];
        ...
        window["emulator_console"] = emulator;

        window["disk"] = function(file) {
	    console.log("main.js disk with arg: "+file);
        };

        window["emulator"] = function(text) {
	    console.log("main.js emulator with arg: "+text);
            //cq.push(text);
            window["emulator_console"].keyboard_send_text(text);
        };


        function service_cq() {
	    console.log("service_cq called");
//            console.log("service_cq queue: "+cq.length);
	    //while (line = cq.shift()) {
//		console.log(line);
...
//	    }	
        ...
        {
	    console.log("Emulator is ready, ID:: "+settings.id);
            if(DEBUG)
            {
            ...
            else if(settings.id == "phogcomputer")
            {
		console.log("Send keys to phogcomputer");
                setTimeout(() => {
                    emulator.keyboard_send_text("\n \
                      rmmod ne2k-pci ; \
                      modprobe ne2k-pci ; \
                      kill 606; \
                      rm -f /var/lib/dhcpcd/enp0s5.lease ; \
                      kill -9 606; \
                      /usr/sbin/dhcpcd -w4 enp0s5; \
                      echo \"\n\nhttps://`cat /sys/class/net/enp0s5/address \
                      | tr : -`.computer.rentals:2015\" > /dev/ttyS0; \n \
                      ")}, 3000);

                //setTimeout(() => {
                //    emulator.serial0_send("jobs; cd /scaffold-eth; clear; ip a; \
                //      until curl http://127.0.0.1:3000 \ndo\nsleep 3\ndone\nip a; yarn generate\n ");
                //      //until yarn generate\ndo\nsleep 3;echo -n .\ndone\nyarn account\n ");
                //}, 25000);
            }
            ...
            if (window["user_script"]) {
                window["user_script"]("init");
            }

            init_ui(settings, emulator);
	    setTimeout(service_cq,3000);
            done && done(emulator);
        });
	    ...
	    console_line += char;
            if(char === "\r")
            {
		console.log("console_line: "+console_line);
                if (window["user_script"]) {
                    window["user_script"](console_line);
                }
		console_line="";
                return;
            }

            document.getElementById("terminal1").value += char;
        });
